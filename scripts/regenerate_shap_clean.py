"""Regenerate a clean SHAP top-10 image and CSV.

This script aggregates existing `logs/shap_*_top20.csv` artifacts, maps engineered
feature names to base names (using `logs/feature_name_map.csv` if available),
filters out the target (`call_flag`, `target`, `y`) and engineered suffixes
(`_lag`, `rollmean`, etc.), and writes a cleaned top-10 CSV and PNG at:

- `logs/shap_top10_clean.csv`
- `logs/shap_top10.png`

Run: .venv\Scripts\python scripts\regenerate_shap_clean.py
"""
import os
import re
import sys
...
def read_shap_series(path):
    # Robust reader: try several heuristics to find which column is feature names
    try:
        # First try: read with index_col=0
    ...
    # If single-column, nothing to do
    if df.shape[1] == 1:
        col = df.columns[0]
        # if values look numeric then it's not useful
        if pd.to_numeric(df[col], errors='coerce').notna().all():
            return None
        # otherwise treat as feature list with implicit ranks (not ideal)
        return pd.Series(np.arange(len(df)), index=df[col].astype(str)).astype(float)

    # Find candidate numeric column (value) and non-numeric (feature)
    numeric_score = {}
    for c in df.columns:
        numeric_score[c] = pd.to_numeric(df[c], errors='coerce').notna().mean()

    # value column likely has high numeric_score; feature column low numeric_score
    value_col = max(numeric_score, key=lambda k: numeric_score[k])
    feature_col = min(numeric_score, key=lambda k: numeric_score[k])

    # If feature_col still looks numeric, try other heuristics: prefer a column named like 'feature' or index-like
...
def base_name(feat, fmap):
    ...
    if feat in fmap:
        return fmap[feat]
    s = str(feat)
    # strip common engineered suffixes like _lag\d+, _rollmean\d+, _rollstd\d+, _p\d+
...
def main():
    ...
    # Remove target and engineered-like names
    ...
    # Canonical allowed features (preserve order if present)
    canonical_allowed = [
    ...
    top = []
    # prefer canonical features in canonical order if they exist
    for f in canonical_allowed:
        if f in agg.index and len(top) < 10:
            top.append(f)

    # fill remaining from agg
    for f in agg.index:
        if len(top) >= 10:
            break
        if f not in top:
